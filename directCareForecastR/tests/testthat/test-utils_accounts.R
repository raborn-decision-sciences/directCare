make_raw_tbl <- function(accounts = "Expenses:Rent",
                         amounts  = 1000,
                         dates    = "01/15/2025") {
  tibble::tibble(
    account = accounts,
...
test_that("default_account_map has required columns", {
  expect_named(default_account_map(), c("pattern", "category", "match_type", "ignore_case"),
               ignore.order = TRUE)
})
...
test_that("default_account_map category values are from the documented set", {
  valid <- c("staff", "supplies", "software", "insurance", "marketing",
             "labs", "equipment", "rent", "licenses", "education", "other")
  m <- default_account_map()
...
test_that("map_accounts maps 'Software' to 'software'", {
  result <- map_accounts(make_raw_tbl("Software Subscription"), default_account_map())
  expect_equal(result$category, "software")
})

test_that("map_accounts maps 'Insurance' to 'insurance'", {
  result <- map_accounts(make_raw_tbl("Malpractice Insurance"), default_account_map())
...
test_that("map_accounts maps 'Equipment' to 'equipment'", {
  result <- map_accounts(make_raw_tbl("Medical Equipment"), default_account_map())
...
test_that("map_accounts maps 'Equipment Insurance' to 'insurance' (insurance rule precedes equipment)", {
  ...
  result <- map_accounts(make_raw_tbl("Equipment Insurance"), default_account_map())
  expect_equal(result$category, "insurance")
})

test_that("map_accounts maps 'Equipment Rental' to 'equipment' (equipment rule precedes rent)", {
  result <- map_accounts(make_raw_tbl("Equipment Rental"), default_account_map())
  expect_equal(result$category, "equipment")
})
...
test_that("map_accounts errors when input is not a data frame", {
  expect_error(map_accounts(list(account = "Rent", amount = 100, date = "01/01/2025"),
                            default_account_map()))
})
