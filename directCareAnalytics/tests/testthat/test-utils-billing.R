# Unit tests for utils_billing.R's .tier_badge()/.tier_info()

test_that(".tier_info resolves the same tier/label .tier_badge renders, for reuse outside the navbar", {
  expect_identical(.tier_info("free"), list(tier = "free", label = "Free"))
  expect_identical(.tier_info("starter"), list(tier = "starter", label = "Starter"))
  expect_identical(.tier_info("pro"), list(tier = "pro", label = "Pro"))
})
