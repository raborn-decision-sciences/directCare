...
test_that("forecast_revenue confidence intervals bound the point forecast", {
  ...
  expect_true(all(
    result$forecast_data$revenue_lower <= result$forecast_data$revenue_forecast
  ))
  expect_true(all(
    result$forecast_data$revenue_forecast <= result$forecast_data$revenue_upper
  ))
})
...
test_that("forecast_revenue ets and arima require the forecast package", {
  ...
  result_arima <- forecast_revenue(
    make_monthly_income(n = 24),
    method = "arima"
  )
